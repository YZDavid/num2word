def num2word(num):
    ...
    for i in breakdown:
        ...
        if i != 0 and i > 999:
            ...
            if prefix in spell:
                output += spell[prefix] + ' ' + spell[suffix] + ', '
            else:
                output += num2word(prefix) + ' ' + spell[suffix] + ', '
        # Non empty hundreds (will include and at the back)
        elif i != 0 and i > 99:
            prefix, suffix = num_slice(i)
            if prefix in spell:
                output += spell[prefix] + ' ' + spell[suffix] + ' and '
            else:
                output += num2word(prefix) + ' ' + spell[suffix] + ' and '
        # Non empty tens when the tens place is '10' as it could contain 'teen' values
        elif i == 10:
            output += spell[sum(breakdown[-2:])] + ' '
            break
        # Non empty digits
        elif i != 0:
            output += spell[i] + ' '
